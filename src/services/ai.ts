...
export async function chatWithAI(
  ...
  const gqlEndpoint = import.meta.env.VITE_GRAPHQL_API_URL as
    | string
    | undefined;
  if (gqlEndpoint) {
    const provider =
      (import.meta.env.VITE_CHAT_PROVIDER as string | undefined) ?? "DEEPSEEK";
    const model =
      (import.meta.env.VITE_CHAT_MODEL as string | undefined) ??
      "deepseek-chat";
    const temperatureRaw = import.meta.env.VITE_CHAT_TEMPERATURE as
      | string
      | undefined;
    ...
    if (typeof reply !== "string")
      throw new Error(
        "GraphQL 返回格式不正确，应在 data.chat.content 提供 string"
      );
    return reply;
  }
  ...
  if (endpoint) {
    ...
    if (typeof reply !== "string")
      throw new Error("API 返回格式不正确，应为 { reply: string }");
    return reply;
  }
